...
int main(void) {
    ...
    while (1) {
        ...
        if (g_save_flag) {
            timer = clock();
            output = personNotPerson((char*)(g_ram_mux ? g_lcd_gram0 : g_lcd_gram1), 320, 240);
            timer = clock() - timer;
            double time_taken = (double) timer / CLOCKS_PER_SEC;
            printf("execution time: %f seconds \n\n", time_taken);

            char time_buffer[100] ;
            sprintf(time_buffer, "execution time: %f seconds", time_taken);

            // lcd_clear(BLACK);
            if(output == 0) {
              lcd_draw_string(16, 80, "Person", GREEN);
              lcd_draw_string(16, 110, time_buffer, BLUE);
            } else {
              lcd_draw_string(16, 80, "Not Person", RED);
              lcd_draw_string(16, 110, time_buffer, BLUE);
            }
            g_save_flag = 0;
            sleep(3);
